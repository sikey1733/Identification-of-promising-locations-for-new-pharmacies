...
rosstat_cheb_people_clean %>% 
  ...
  ggplot(aes(x = reorder(Городской_округ, -Численность), y = Численность,
             fill = Категория)) +
...
### Конкурентный анализ (выборочный)
...
### Поиск потенциальных мест (близость к обьектам ифроструктуры, отсутствием конкурентов поблизости)
...
result_residential <- final_table %>%
  ...
  filter(
    ...
    !is.na(lon), !is.na(lat), !is.na(name), name != ""
  ) %>%
  rowwise() %>%
  mutate(
